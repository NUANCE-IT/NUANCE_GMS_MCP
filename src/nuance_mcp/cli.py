...
def main(argv: list[str] | None = None) -> None:
    p = argparse.ArgumentParser(
        ...
        description="Vendor-agnostic MCP server for multimodal electron "
                    "microscopy. Built-in adapters: "
                    + ", ".join(available_adapters()) + ".",
    )
    sub = p.add_subparsers(dest="cmd", required=True)

    serve = sub.add_parser("serve", help="Run the MCP server.")
    serve.add_argument("--adapter", default="simulator",
                       help="Adapter name (default: simulator).")
    ...
    p.add_argument("--version", action="version",
                   version=f"nuance-mcp {__version__}")
    ...
    if args.cmd == "serve":
        ...
        run_server(adapter_name=args.adapter,
