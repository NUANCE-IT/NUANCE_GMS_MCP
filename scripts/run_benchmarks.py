...
try:
    ...
    print(
        f"Error importing nuance_mcp: {e}\nDid you install the [ollama] extra?",
        file=sys.stderr,
    )
    sys.exit(1)
    ...
    for i in range(runs):
        ...
        try:
            ...
            reply = await run_agent(
                server, BENCHMARK_PROMPT, model=model, temperature=0.0
            )
...
async def main():
    ...
    for m, d in results.items():
        mean_str = (
            f"{d['mean_latency']:.2f} s" if d["mean_latency"] is not None else "N/A"
        )
        logging.info(
            f"  {m:<20} {d['success_count']:>2d}/{d['total_runs']} successful, mean: {mean_str}"
        )
